(function() {
  ...
  Crafty.c("TiledLevel", {
    ...
    makeTiles: function(ts, drawType) {
      var components,desc, i, posx, posy, sMap, sName, tHeight, tName, tNum, tWidth, tsHeight, tsImage, tsProperties, tsWidth, xCount, yCount, _ref;
      ...
      for (i = 0, _len = lData.length; i < _len; i++) {
        ...
        if (tDatum) {
		  ...
		  if(layer.name =="objects") {
          	this.blockedtiles.push(tile.x+","+tile.y);
		  }	
		  this.alltiles.push(tile.x+","+tile.y);
		}
      }	
	  this.loaded = true;
      return null;
